...
kony.sdk.error.getIntegrationErrObj = function(errResponse) {
	try	{
		...
		if(service) {
			errorMessagePrefixForIntegration = "Integration Service Request Failed for " + serviceName + ":";
		} else {
...
kony.sdk.prototype.getMetricsService = function () {
	...
	if(this.internalSdkObject){
		...
		metricsServiceObject =  new MetricsService(this);
	}

	try{
		kony.license.registerSessionIDListener(metricsServiceObject);
		return metricsServiceObject;
	}
	catch(e){
		throw new Exception(Errors.METRICS_FAILURE, ""+e);
	}
...
//start of event api
	...
	var eventConfig = {"confType" : "BUFFER","eventBufferAutoFlushCount": eventBufferAutoFlushValue,"eventBufferMaxCount" : eventBufferMaxValue};
	var reportEventBufferArray = [];
	var reportEventBufferBackupArray = [];
	var retrievedDS = false;
	var eventBufferCount = 0;
	var eventTypeMap = {"formentry" : "FormEntry","touch" : "Touch","servicecall" : "ServiceCall","gesture" : "Gesture","orientation" : "Orientation","custom" : "Custom"};
	var errorCodeMap = {"1000" : true,"1011" : true,"1012" : true,"1014" : true,"1015" : true,"1016":true};
	...
	this.reportError = function(errorCode, errorType, errorMessage, errorDetails){
		...
		metaData.errmsg = errorMessage;
		metaData.errfile = errorDetails.errfile;
		metaData.errmethod = errorDetails.errmethod;
		metaData.errline = errorDetails.errline;
		metaData.errstacktrace = errorDetails.errstacktrace;
		metaData.errcustommsg = errorDetails.errcustommsg;
		var formID = errorDetails.formID;
		var widgetID = errorDetails.widgetID;
		var flowTag = errorDetails.flowTag;
		var evtSubType = errorType;
	...
	this.reportHandledException =function(exceptioncode,exceptionType,exceptionMessage,exceptionDetails){
		var metaData = {};
		metaData.exceptioncode = exceptioncode;
		metaData.exceptionmsg = exceptionMessage;
		metaData.errfile = exceptionDetails.errfile;
		metaData.errmethod = exceptionDetails.errmethod;
		metaData.errline = exceptionDetails.errline;
		metaData.errstacktrace = exceptionDetails.errstacktrace;
		metaData.errcustommsg = exceptionDetails.errcustommsg;
		var formID = exceptionDetails.formID;
		var widgetID = exceptionDetails.widgetID;
		var flowTag = exceptionDetails.flowTag;
		var evtSubType = exceptionType;
